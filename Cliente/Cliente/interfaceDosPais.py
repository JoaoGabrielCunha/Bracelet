...
def enviar_mensagem():
    ...
    print(mensagem)
    cliente_pais.enviar_mensagem(mensagem)
...
def funcao_botao1():
    ...
    with open("emojis_salvos.json", "r") as arquivo2:
        ...
        if simbolo_correspondente:
            ...
            print(nome_emoji)
        cliente_pais.enviar_mensagem(nome_emoji)
def funcao_botao2():
    with open("mensagens_alteradas.json", "r") as arquivo1:
        dados1 = json.load(arquivo1)
        print(dados1[1])
...
def display_de_alerta(msg):
    if msg != "" and msg is not None:
        etiqueta_alerta = tk.Label(janela, text="Mensagem de alerta:", font=("Arial", 10, "bold"))
        etiqueta_alerta.place(x=600, y=20)
        
        global msg_alerta
        msg_alerta = tk.Label(janela, text=msg, font=("Arial", 15, "bold"), fg="red")
        msg_alerta.place(x=620, y=40)
        
        
        # Função para alternar entre mostrar e esconder o texto
        def alternar_texto():
            global cont
            cont+=1
            if(cont < 7):
                if msg_alerta.cget("text") == "":
                    msg_alerta.config(text=msg)
                else:
                    
                    player.loadfile("beep-06.mp3")
                    msg_alerta.config(text="")
                # Agendando próxima alternância após 1 segundo (1000 milissegundos)
                janela.after(1000, alternar_texto)
        
        # Iniciando a alternância
        alternar_texto()
    else:
        # Se mensagem for vazia ou None, esconde a label
        if 'msg_alerta' in globals():
            msg_alerta.destroy()
...
def pinga_localizacao():
    ...
    timestamp_completo = time.strftime("%d-%m-%Y %H:%M:%S", time.localtime())
    marker_loc = map_widget.set_marker(x, y, text=f"{timestamp}")
    etiqueta_ultimo_momento = tk.Label(janela, text=f"Ultima localização: {timestamp_completo}")
    etiqueta_ultimo_momento.place(x=300, y=220)
...
def tratamento_mensagem(msg):
    print("OK OK")
    if(msg=="help"):
        print("Entrou aqui!")
        display_de_alerta(msg)
        

def mensagem_recebida():
    while True:
        time.sleep(1)
        #print("msg = " + str(cliente_pais.get_msg()))
        if cliente_pais.get_msg() != None:
            print("Não é None")
            msg = cliente_pais.get_msg()
            cliente_pais.set_msg(None)
            tratamento_mensagem(msg)
        


thread = Thread(target=mensagem_recebida)
thread.daemon = True
thread.start()  
